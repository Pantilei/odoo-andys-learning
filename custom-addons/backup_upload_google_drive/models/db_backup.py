...
class DbBackup(models.Model):
    ...
    def set_values(self):
        ...
        authorization_code_before = params.get_param(
        ...
        for rec in conf_ids:
            ...
            if rec.name in db_list:
                ...
                date_today = pytz.utc.localize(
                    datetime.datetime.today()).astimezone(user_tz)
                bkp_file = '%s_%s.%s' % (rec.name, date_today.strftime(
                    '%Y-%m-%d_%H_%M_%S'), rec.backup_type)
    ...
    def google_drive_upload(self, rec, file_path, bkp_file):
        ...
        if rec.is_upload:
            ...
            if r.status_code == 200:
                ...
                notification_template = self.env.ref(
                    'backup_upload_google_drive.email_google_drive_upload')
        ...
        if rec.drive_autoremove:
            ...
            url = "/drive/v3/files"
            status, content, ask_time = self.env['google.service']._do_request(
                url, params, headers, method='GET')

            for item in content['files']:
                date_today = datetime.datetime.today().date()
                create_date = datetime.datetime.strptime(
                    str(item['createdTime'])[0:10], '%Y-%m-%d').date()
                ...
                if delta.days >= rec.drive_to_remove:
                    ...
                    response = self.env['google.service']._do_request(
